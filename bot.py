...
async def telegram_worker():
    ...
    while True:
        bot_username, message, future = await message_queue.get()
        try:
            now = time.time()
            # Tekrarlanan mesaj kontrolü (bot bazlı)
            key = f"{bot_username}:{message}"
            if key in latest_sent and now - latest_sent[key] < DUPLICATE_WINDOW:
...
async def send_message_queue(bot_username: str, message: str):
    ...
    await message_queue.put((bot_username, message, future))
    return await future
