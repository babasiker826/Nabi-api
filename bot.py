...
def run_async(coro):
    # Render ve benzeri platformlarda zaten loop olabilir, güvenli çalışması için
    try:
...
def chat():
    ...
    try:
        reply = run_async(send_and_wait_followup(message, wait_seconds=10))
...
if __name__ == "__main__":
    ...
    app.run(host="0.0.0.0", port=port)        # 0.0.0.0 olmalı ki dışarıdan erişilebilsin
